...
describe Canto do
  ...
  it { canto = Canto.new.should_not allow_value("", nil).for(:title) }
  ...
  it "has mandatory title" do 
    FactoryGirl.build(:canto, title: nil).should_not be_valid #Factory.build() instantiates a new model, but doesn’t save it
  end
  ...
  it { canto = Canto.new(no:1,title:1).should_not allow_value("", nil).for(:book_id) }
end